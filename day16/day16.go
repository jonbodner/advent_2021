...
func read3(s *string) int {
	val, _ := strconv.ParseInt((*s)[:3], 2, 8)
	*s = (*s)[3:]
	return int(val)
}

var hexLookup = map[byte]string{
	'0': "0000",
	'1': "0001",
	'2': "0010",
	'3': "0011",
...
func eval(packet Packet) int {
	...
	case 0:
		total := 0
		for _, v := range packet.SubPackets {
			total += eval(v)
		}
		return total
	case 1:
	...
	case 3:
		...
		return max
	case 5:
		if eval(packet.SubPackets[0]) > eval(packet.SubPackets[1]) {
			return 1
		}
		return 0
	case 6:
