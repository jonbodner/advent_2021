...
Given the starting energy levels of the dumbo octopuses in your cavern, simulate 100 steps. How many total flashes
are there after 100 steps?
*/
...
func part1() {
	...
	for i := 0; i < 100; i++ {
		fmt.Println(i + 1)
		increment(start)
		fmt.Println("after initial increment")
		printBoard(start)
		for {
			count := flash(start)
			fmt.Println("number popped", count)
			total += count
			if count == 0 {
				break
			}
...
func part2() {
	...
	printBoard(start)
	count := 0
	boardSize := len(start) * len(start[0])
loop:
	for {
		fmt.Println(count + 1)
		increment(start)
		...
		for {
			popped := flash(start)
			fmt.Println("number popped", popped)
			totalPopped += popped
			if totalPopped == boardSize {
				break loop
			}
			if popped == 0 {
				break
...
func getInitial() [][]byte {
	contents, _ := os.ReadFile("./day11/input.txt")
	//	contents = []byte(`5483143223
	//2745854711
	//5264556173
	//6141336146
	//6357385478
